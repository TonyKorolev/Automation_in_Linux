...
def make_subfolder():
    ...
    if not ssh_checkout(data["host"], data["user"], data["password"], "cd {}; mkdir {}".format(data['folder_in'], subfoldername), ""):
        return None, None
...
def make_bad_arx(make_folders, clear_folders, make_files):
    ssh_checkout(data["host"], data["user"], data["password"], "cd {}; 7z a {}/arx1.7z".format(data['folder_in'], data['folder_badarx']),
                      "Everything is Ok"), "Test bad Fail"
    return ssh_checkout(data["host"], data["user"], data["password"], "truncate -s {}/badarx.7z".format(data['folder_badarx']), ""), "test FAIL"
